...
def get_predictions_all_path(output: str | Path) -> Path:
    return Path(output) / 'predictions_all.npz'


def dump_predictions_all(
    output: str | Path, predictions_all: dict[PartKey, np.ndarray]
) -> None:
    np.savez(get_predictions_all_path(output), allow_pickle=True, **predictions_all)
